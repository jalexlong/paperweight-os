#!/usr/bin/env bash
# Manual/local publish path — NOT part of the normal workflow. CI
# (.github/workflows/build-and-publish.yml) builds, lints, and publishes
# every packaging/*/debian/control package to gh-pages automatically on
# push to main. Only run this yourself for a one-off local publish (e.g.
# testing signing, or publishing outside of CI) — invoke with purpose.
set -euo pipefail
