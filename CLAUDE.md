...
fastfetch/
  ...
  paperweight-logo.png          — copied by fastfetch-theme-gen.py from paperweight-plymouth's
                                   rock.png; rendered via fastfetch's "chafa" logo type with
                                   "fgOnly": true so the terminal's own translucent background
                                   shows through (see THEME-AUTHORING.md §9); theme-agnostic
  themes/<name>.jsonc            — per-theme text colors + module list, generated by
                                   fastfetch-theme-gen.py (repo root) from the sway palette
.local/bin/
